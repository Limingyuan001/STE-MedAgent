...
if __name__ == "__main__":
    ...
    openai_kwargs = {}
    api_key = os.getenv("OPENAI_API_KEY") or os.getenv("DASHSCOPE_API_KEY") # 提供api_key，用于访问openai或dashscope
    if api_key:
        openai_kwargs["api_key"] = api_key

    if base_url := os.getenv("OPENAI_BASE_URL"):
        openai_kwargs["base_url"] = base_url # 提供base_url，用于访问openai或dashscope

    # Allow overriding the model via env (e.g., qwen-vl-plus)
    model = os.getenv("OPENAI_MODEL") # 默认调用.env中的OPENAI_MODEL=qwen3-vl-plus，如果需要调用其他模型，可以在.env中修改OPENAI_MODEL，url，api_key。

    prompt_file = os.path.join(os.path.dirname(__file__), "medrax", "docs", "system_prompts.txt")
